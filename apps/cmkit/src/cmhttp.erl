...
-record(state, {app, handler, req, action, spec, user, params}).


routes(App, Handler) -> 
  State = #state{app = App, handler = Handler},
  [ 
    {"/api/[...]", ?MODULE, State},
    {"/ws", ?MODULE, State},
...
websocket_handle({text, Text}, #state{app=App, handler=Handler, params=Input0}=State) ->
    ...
    #{<<"action">> := Action} = Json ->
      ...
      case Specs of
        ...
        [{_Role, Params, Act}=Spec] ->
          ...
          State3=State2#state{params=Input1, spec=Spec},
          case cmkit:parse(Params, Input1) of
            {_, K} ->
              ws_invalid(K, State3);
            Input2 -> 
