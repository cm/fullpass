...
open(_Name, _Type, _Size) -> 
    %Opts = opts(Type),
    Workdir = cmkit:config(workdir, cmdb),
    FullDir = string:join([Workdir, "cow.db"], "/"),
    {ok, Pid} = cowdb:open(FullDir),
    {ok, undef, Pid}.
...
write(Pid, K, V) when is_binary(K) ->
    %Vbin = erlang:term_to_binary(V),
    Vbin = cmkit:jsone(V),
    cmkit:log({?MODULE, K, Vbin}),
    {ok, _} = cowdb:put(Pid, K, Vbin),
    ok.
